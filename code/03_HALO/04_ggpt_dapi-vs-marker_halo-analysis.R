...
get_sm_plots <- function(plot.str, expt, cname.marker, fnv, lcsv, ext = "png"){
  ...
  # save new plot
  plot.fname <- paste0("ggsm-composite_dapi-nuc-vs-",
                       plot.str,"_halo-analysis.",ext)
  ggsave(filename = plot.fname, plot = ggsm, width = 20, height = 20, 
         dpi = 400, units = "in")
}
...
cnv <- c("GAD1..Opal.690..Nucleus.Intensity", "GFAP..Alexa.594..Cell.Intensity",
...
dfp <- do.call(rbind, lapply(fnvf, function(fni){
  ...
  dfi$file <- gsub(".*\\/", "", fni); dfi
}))

# get density plot object
ggsm <- ggplot(dfp, aes(y = marker, x =  dapi, 
                        fill = marker.name, color = marker.name)) + 
  geom_smooth() + theme_bw() + facet_wrap(~file)
