...
TSNE_cellTypes_broad <- ggcells(sce, mapping = aes(x = TSNE.1, y = TSNE.2, colour = cellType_broad_hc)) +
  ...
  scale_color_manual(values = cell_type_colors_broad, name = "Cell Type") +
  coord_equal() +
  labs(x = "TSNE Dimension 1", y = "TSNE Dimension 2") +
  my_theme +
  guides(color = guide_legend(override.aes = list(size = 2, alpha = 1)))

ggsave(TSNE_cellTypes_broad, filename = here(plot_dir, "TSNE_cellTypes_broad.png"), width = 5, height = 3.75)
