...
DREAM_library_prep <- map2(rse_list, names(rse_list), function(rse, type_name){
  ...
  map2(prep_pairs, names(prep_pairs), function(pair, pair_name){
    ...
    message("coef[2]:", colnames(fitmm$design)[[2]])
    
    outDE <- topTable( fitmm, coef=2, number=Inf , sort.by = "none")
    
    message(Sys.time(), " - Saving")
    try(write.csv(outDE, file = here(data_dir, paste0("DREAM_library-prep_gene_",type_name,"_",pair_name,".csv")), row.names = TRUE))
...
save(DREAM_library_prep, file = here(data_dir, "DREAM_library-prep.Rdata"))
